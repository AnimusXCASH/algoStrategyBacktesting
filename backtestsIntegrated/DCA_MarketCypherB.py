...
from strategies.dca_market_cypher_B import DCAMarketCypherB
...
if __name__ == "__main__":
    ...
    # cerebro.addstrategy(
    #     DCAMarketCypherB,
    #     initial_trade_size_percentage=4,
    #     wt_n1=14,
    #     wt_n2=17,
...
####################################### OPTIMIZATION ##############################################################
    ...
    pnl_perc_limit = 6  # PNL > pnl_perc_limit


    # Constructing the table
    all_keys = ["initial_trade_size_percentage", "wt_n1", "wt_n2",
                "take_profit_percentage", "Max Drawdown (%)", "Transactions", "PNL%"]
    
    table.field_names = ["Strategy"] + all_keys


    # price_drop_percentage_range = np.arange(1.0, 6.0, 2.0)
    initial_trade_size_percentage_range = np.arange(4,8,2)
    wt_1_range = np.arange(6,20,2)
    wt_2_range = np.arange(8,26,2)
    take_profit_percentage_range = np.arange(2.0, 6.0,2.0)

    # Calculate the number of values for each parameter
    # num_price_drop_percentage = len(price_drop_percentage_range)
    num_initial_trade_size_percentage_range = len(initial_trade_size_percentage_range)
    num_wt_1_range = len(wt_1_range)
    num_wt_2_range = len(wt_2_range)
    num_take_profit_percentage_range = len(take_profit_percentage_range)

    # Calculate the total number of tests
    total_tests = (num_initial_trade_size_percentage_range *
                num_wt_1_range *
                num_wt_2_range *
                num_take_profit_percentage_range)
