...
def plot_chart(i, symbol, df, current_price, buy_grid, sell_grid, buy_stop_loss, sell_stop_loss):
    ...
    #  Create sub plots
    fig = make_subplots(rows=1, cols=1, subplot_titles=[f"{i} {symbol} Chart", ], specs=[[{"secondary_y": False}]],
...
class GridStrategy(Strategy):
    ...
    atr_len = 1
    num_grid_lines = 5
    grid_interval = 10 / 10000  # number of pips
...
def run_backtest(df):
    bt = Backtest(df, GridStrategy, cash=500, commission=.00075, trade_on_close=True, exclusive_orders=False,
                  hedging=False)
    stats = bt.run()
    print(stats)
    bt.plot()
